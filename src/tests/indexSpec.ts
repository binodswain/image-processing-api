...
describe("Endpoint testing", () => {
    ...
    it('GET req with valid params', async () => {
        const response = await request.get('/api/images?filename=palmtunnel&height=700&width=800');
        expect(response.status).toBe(200);
    ...
    it('GET req with invalid filename', async () => {
        const response = await request.get('/api/images?filename=invalid&height=700&width=800');
    ...
    it('GET req with only height param', async () => {
        const response = await request.get('/api/images?filename=palmtunnel&height=700');
        expect(response.status).toBe(200);
    ...
    it('GET req with only width param', async () => {
        const response = await request.get('/api/images?filename=palmtunnel&width=800');
        expect(response.status).toBe(200);
