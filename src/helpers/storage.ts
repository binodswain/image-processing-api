 ...
 * @returns boolean value
 */
export const doesThumbnailExist = async (
    filename: string
): Promise<boolean> => {
    try {
 ...
 * @returns array of image object with filename and url
 */
export const getThumbnailList = async (): Promise<
    Array<{
        filename: string;
        url: string;
        meta: {
...
async function getFilenameMap(): Promise<{ [key: string]: string }> {
    ...
    const filenameMap = filelist.reduce(
        (
            acc: {
                [key: string]: string;
            },
...
async function getThumbnailMap(): Promise<{ [key: string]: string }> {
    ...
    const filenameMap = filelist.reduce(
        (
            ...
            file: {
                filename: string;
                meta: { filename: string; height: string; width: string };
            }
        ) => {
            acc[
                `${file.meta.filename}-${file.meta.height}x${file.meta.width}`
            ] = file.filename;
            return acc;
        },
        {}
